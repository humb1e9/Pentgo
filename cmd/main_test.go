...
func isolateConfig(t *testing.T) string {
	...
	data := []byte(`{"model":{"provider":"openai","base_url":"https://api.openai.com/v1","model":"fixture","api_key":"fixture"},"tools":{},"project":{}}`)
...
func TestRunCommandResumeRequiresExistingWorkspace(t *testing.T) {
	...
	t.Chdir(workspace)
	var stdout, stderr bytes.Buffer
	if code := runCommand(context.Background(), []string{"resume"}, strings.NewReader(""), &stdout, &stderr); code != 1 {
		t.Fatalf("exit code = %d; stdout/stderr = %q/%q", code, stdout.String(), stderr.String())
	}
