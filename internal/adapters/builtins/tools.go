...
func NewTools(workspace *Workspace) []agent.Tool {
	return []agent.Tool{
		&workspaceTool{workspace: workspace, name: "ls", description: "List files and directories in the workspace.", schema: objectSchema(map[string]any{"path": stringProperty("Workspace-relative directory")}), invoke: invokeLS},
		&workspaceTool{workspace: workspace, name: "read_file", description: "Read text from a workspace-relative file.", schema: objectSchema(map[string]any{"file_path": stringProperty("Workspace-relative file path"), "offset": integerProperty("1-based starting line"), "limit": integerProperty("Maximum number of lines; zero means all")}, "file_path"), invoke: invokeRead},
		&workspaceTool{workspace: workspace, name: "write_file", description: "Write text to a workspace-relative file.", schema: objectSchema(map[string]any{"file_path": stringProperty("Workspace-relative file path"), "content": stringProperty("Complete file content")}, "file_path", "content"), invoke: invokeWrite},
		&workspaceTool{workspace: workspace, name: "edit_file", description: "Replace text in a workspace-relative file.", schema: objectSchema(map[string]any{"file_path": stringProperty("Workspace-relative file path"), "old_string": stringProperty("Exact text to replace"), "new_string": stringProperty("Replacement text"), "replace_all": booleanProperty("Replace every occurrence")}, "file_path", "old_string", "new_string"), invoke: invokeEdit},
		&workspaceTool{workspace: workspace, name: "glob", description: "Find workspace files matching a glob pattern.", schema: objectSchema(map[string]any{"pattern": stringProperty("Glob pattern"), "path": stringProperty("Workspace-relative base directory")}, "pattern"), invoke: invokeGlob},
		&workspaceTool{workspace: workspace, name: "grep", description: "Search workspace files with a regular expression.", schema: objectSchema(map[string]any{"pattern": stringProperty("Regular expression"), "path": stringProperty("Workspace-relative base directory"), "glob": stringProperty("File glob filter"), "file_type": stringProperty("File type filter"), "case_insensitive": booleanProperty("Case-insensitive search"), "enable_multiline": booleanProperty("Enable multiline search"), "after_lines": integerProperty("Lines after each match"), "before_lines": integerProperty("Lines before each match")}, "pattern"), invoke: invokeGrep},
