...
// Record 追加证据并返回其稳定序号。
func (store *EvidenceStore) Record(ctx context.Context, tool string, arguments map[string]any, success bool, output string) (int, error) {
	record, err := store.RecordResult(ctx, tool, arguments, success, output)
	if err != nil {
		return 0, err
	}
