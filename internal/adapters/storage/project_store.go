...
func createProjectStore(root, name, projectID string, now time.Time) (*ProjectStore, error) {
	...
	project := &domain.Project{ID: strings.TrimSpace(projectID), Name: name, CreatedAt: now, UpdatedAt: now}
...
func (store *ProjectStore) LoadProject() (*domain.Project, error) {
	...
	defer rows.Close()
	project.Sessions = []domain.SessionSummary{}
	for rows.Next() {
...
}, id string) (*domain.Session, error) {
...
func loadTurn(queryer interface{ QueryRow(string, ...any) *sql.Row }, id string) (*domain.Turn, error) {
	...
	turn.Status = domain.TurnStatus(status)
...
func (store *ProjectStore) SaveSession(session *domain.Session) error {
...
// 确保可重建的项目索引不会领先于会话状态。
func (store *ProjectStore) CommitSession(session *domain.Session, project *domain.Project) error {
...
// 它始终在调用方持有的事务中执行。
func saveSessionTx(tx *sql.Tx, session *domain.Session) error {
...
func (store *ProjectStore) SaveProjectIndex(project *domain.Project) error {
...
func (store *ProjectStore) saveProject(project *domain.Project) error {
	tx, err := store.db.Begin()
	if err != nil {
...
func saveProjectTx(tx *sql.Tx, project *domain.Project) error {
...
// SaveBlackboard 原子替换项目范围的共享事实。
func (store *ProjectStore) SaveBlackboard(board *domain.Blackboard) error {
	if store == nil || store.db == nil || board == nil {
		return fmt.Errorf("blackboard is invalid")
	}
	store.commitMu.Lock()
	defer store.commitMu.Unlock()
	tx, err := store.db.Begin()
	if err != nil {
		return fmt.Errorf("begin blackboard save: %w", err)
	}
	if err := saveBlackboardTx(tx, board); err != nil {
		_ = tx.Rollback()
		return err
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit blackboard save: %w", err)
	}
	return nil
}

// saveBlackboardTx 按显式位置顺序替换规范化事实，保持模型和用户看到的顺序一致。
func saveBlackboardTx(tx *sql.Tx, board *domain.Blackboard) error {
	if _, err := tx.Exec("DELETE FROM facts"); err != nil {
		return fmt.Errorf("replace blackboard facts: %w", err)
	}
	for position, fact := range board.Facts {
		fact.Key = strings.TrimSpace(fact.Key)
		fact.Value = strings.TrimSpace(fact.Value)
		if fact.Key == "" || fact.Value == "" {
			return fmt.Errorf("blackboard fact key and value are required")
		}
		if fact.At.IsZero() {
