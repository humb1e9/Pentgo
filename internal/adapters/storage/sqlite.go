...
CREATE UNIQUE INDEX IF NOT EXISTS context_compactions_one_started_range
	...
	if version <= 6 {
		tx, err := db.Begin()
		if err != nil {
			return fmt.Errorf("begin sqlite schema v6 to v7 migration: %w", err)
		}
		if _, err := tx.Exec(`
DROP TABLE IF EXISTS project_fact_edges;
DROP TABLE IF EXISTS project_fact_evidence;
...
CREATE TABLE project_facts (
    ...
    PRIMARY KEY(project_id, fact_key)
			...
			_ = tx.Rollback()
			return fmt.Errorf("migrate sqlite schema v6 to v7: %w", err)
		}
		if err := tx.Commit(); err != nil {
			return fmt.Errorf("commit sqlite schema v6 to v7 migration: %w", err)
		}
	}
	return nil
}
