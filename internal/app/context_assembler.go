...
func (assembler *ContextAssembler) PrepareOverflowRecovery(ctx context.Context, sessionID, systemPrompt string, tools []agent.Tool, facts string) (agent.ModelStepInput, []agent.ContextActivity, error) {
	...
	checkpointed, checkpointActivities, err := compactor.CheckpointWithValidator(ctx, CompactionRequest{Surface: pruned, Messages: messages, SystemPrompt: systemPrefix, ProjectFacts: factsEnvelope, Tools: tools, Prunes: prunes}, func(candidate agent.ContextSurface) error {
...
func (assembler *ContextAssembler) Prepare(ctx context.Context, sessionID, systemPrompt string, tools []agent.Tool, facts string) (agent.ModelStepInput, []agent.ContextActivity, error) {
	if assembler == nil || assembler.runtime == nil {
		return agent.ModelStepInput{}, nil, fmt.Errorf("context assembler dependencies are incomplete")
	}
	if ctx == nil {
	...
	messages := numberedMessages(transcriptMessages)
	factsEnvelope := llm.ProjectFactsEnvelope(facts)
	activities := make([]agent.ContextActivity, 0, 2)
	...
	fixed := assembler.meter.Measure(ContextRequest{SystemPrompt: systemPrefix, ProjectFacts: factsEnvelope, Tools: tools})
...
func contextRequestFromInput(input agent.ModelStepInput) ContextRequest {
	...
	return ContextRequest{SystemPrompt: llm.SystemInstructionPrefix(input.SystemPrompt), ProjectFacts: llm.ProjectFactsEnvelope(input.ProjectFacts), Tools: input.Tools, Nodes: nodes, Messages: messages}
}
