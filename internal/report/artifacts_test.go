...
func artifactTestSession(t *testing.T) *sess.AgentSession {
	...
	session := sess.NewSession(sess.Target{Raw: "https://example.test", Canonical: "https://example.test"}, "检查目标", started)
...
func TestEngagementWriterPublishesRuntimeSessionReportEvidenceAndWorkDir(t *testing.T) {
	...
	var decoded sess.AgentSession
	if err := json.Unmarshal(data, &decoded); err != nil {
		t.Fatal(err)
	}
	if decoded.Status != sess.SessionDone || decoded.StopReason != "task_complete" {
...
func TestWriteArtifactsPublishesCancelledRuntimeSession(t *testing.T) {
	session := artifactTestSession(t)
	session.Status = sess.SessionRunning
