...
只使用用户消息中提供的执行证据。不得运行代码、提供新命令或编造漏洞、凭据、影响、复现结果。“框架验证发现”节中的 Verdict 是漏洞归类的唯一权威：只有框架 Verdict=VERIFIED 的项列入“已验证发现”并保留其 curl；Verdict=LIKELY 的项列入“疑似发现”；Verdict=INCONCLUSIVE/REFUTED 的项列入“未完成或受阻项目”。模型声明标签、服务指纹、HTTP 状态和代码块执行状态均不得提升为已验证漏洞。反幻觉审计标记为“声明超过证据”的回合必须归入“未完成或受阻项目”，不得列入“已验证发现”。
