...
func TestEvidenceLevelVocabulary(t *testing.T) {
	for _, level := range []EvidenceLevel{EvidenceVerified, EvidenceLikely, EvidenceInferred} {
		if string(level) == "" {
			t.Fatalf("empty evidence level")
		}
	}
	if EvidenceVerified != "VERIFIED" || EvidenceLikely != "LIKELY" || EvidenceInferred != "INFERRED" {
		t.Fatalf("unexpected vocabulary: %s %s %s", EvidenceVerified, EvidenceLikely, EvidenceInferred)
	}
}
