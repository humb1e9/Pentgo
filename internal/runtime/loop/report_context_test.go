...
func TestRunnerCopiesFrameworkFindingsIntoReportContext(t *testing.T) {
	...
	runner.findings = []verify.VerificationResult{{
	...
	session := sess.NewSession(sess.Target{Canonical: "https://example.test"}, "检查首页", time.Now().UTC())

	reportContext := runner.ReportContext(session)
	if len(reportContext.VerifiedFindings) != 1 || reportContext.VerifiedFindings[0].Verdict != verify.VerdictLikely {
