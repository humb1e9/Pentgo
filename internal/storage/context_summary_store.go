...
func (store *ProjectStore) LoadContextSummary(ctx context.Context, sessionID string) (projectmodel.ContextSummary, bool, error) {
	if store == nil || store.db == nil {
		return projectmodel.ContextSummary{}, false, fmt.Errorf("project store is nil")
	}
	if ctx == nil {
	...
	if sessionID == "" {
		return projectmodel.ContextSummary{}, false, fmt.Errorf("summary session ID is empty")
	}
	var summary projectmodel.ContextSummary
	var updatedAt int64
	...
	if err == sql.ErrNoRows {
		return projectmodel.ContextSummary{}, false, nil
	}
	if err != nil {
		return projectmodel.ContextSummary{}, false, fmt.Errorf("load context summary: %w", err)
	}
...
func (store *ProjectStore) SaveContextSummary(ctx context.Context, summary projectmodel.ContextSummary) error {
	if store == nil || store.db == nil {
