...
- [web-cache-deception](web-cache-deception.md) when Host injection is combined with cache behavior
- [ssrf-server-side-request-forgery](ssrf-server-side-request-forgery.md) when Host header routes requests to internal services
- [open-redirect](open-redirect.md) when Host injection causes redirect to attacker domain
- waf-bypass-techniques when Host manipulation helps bypass WAF routing
- [request-smuggling](request-smuggling.md) when smuggling enables Host header manipulation past front-end validation
- [subdomain-takeover](subdomain-takeover.md) when Host routing exposes internal vhosts resolvable via subdomain
