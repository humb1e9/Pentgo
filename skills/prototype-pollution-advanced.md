...
> **AI LOAD INSTRUCTION**: Advanced prototype pollution escalation. Covers server-side RCE via template engines (EJS, Pug, Handlebars), Node.js child_process gadgets, client-side script gadgets, filter bypass patterns, and systematic detection. Load [prototype-pollution.md](prototype-pollution.md) first for fundamentals (merge sinks, `__proto__` vs `constructor.prototype`, basic probes).

## 0. RELATED ROUTING

- [prototype-pollution](prototype-pollution.md) — **LOAD FIRST** for PP fundamentals, merge-sink detection, basic probes
- [ssti-server-side-template-injection](ssti-server-side-template-injection.md) — template engine RCE context (PP often triggers through template gadgets)
- [xss-cross-site-scripting](xss-cross-site-scripting.md) — client-side PP gadgets ultimately achieve XSS
...
```
1. Find merge sink (prototype-pollution.md Section 0)
...
│
└── Not confirmed yet?
    └── Go back to prototype-pollution.md for detection
```
