...
- [ssrf-server-side-request-forgery](ssrf-server-side-request-forgery.md) when these services are reachable via SSRF (e.g., SSRF → Redis)
- [jndi-injection](jndi-injection.md) when H2 Console or similar accepts JNDI connection strings
- [deserialization-insecure](deserialization-insecure.md) when RMI Registry or T3 protocol is exposed
...
Also see [jndi-injection](jndi-injection.md) for the full JNDI/BeanFactory exploitation flow.
